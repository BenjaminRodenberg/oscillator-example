...
def do_run(precice_config_params=default_precice_config_params, experiment_params={}):
    experiment_folder = Path(__file__).parent.absolute()
    executable = "oscillator.py"
    ...
    for participant in participants:
        with open(experiment_folder / participant['logfile'], "w") as outfile:
            cmd = ["python3",
                   experiment_folder / executable,
                   participant["cmd"]] + [f"{opt}={value}" for opt, value in experiment_params.items()]
            p = subprocess.Popen(cmd,
                                 cwd=experiment_folder,
                                 stdout=outfile)
            participant["proc"] = p
    ...
    for participant in participants:
        df = pd.read_csv(experiment_folder / f"errors-{participant['name']}.csv", comment="#")
