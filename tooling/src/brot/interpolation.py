...
def make_cubic_spline(p0, m0, p1, m1, dt):
    ...
    """
    # return lambda t : (2*t**3 - 3*t**2 + 1) * p0 + (t**3 - 2*t**2 + t) * dt
    # * m0 + (-2*t**3+3*t**2) * p1 + (t**3 - t**2) * dt * m1
    return lambda t: (((2 * p0 - 2 * p1 + dt * m0 + dt * m1) * t + (- 3 * p0 -
                      2 * dt * m0 - dt * m1 + 3 * p1)) * t + m0 * dt) * t + p0
