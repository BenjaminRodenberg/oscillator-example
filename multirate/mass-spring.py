...
parser.add_argument("-is", "--interpolation-scheme", help="Interpolation scheme being used.", type=str, default=ReadWaveformSchemes.LAGRANGE_LINEAR.value)
...
for dt in dts:
    # We support only a single multirate case here, where Mass-Left subcycles and Mass-Right has a fixed dt.
    # use fixed dt for preCICE and Mass-Left
    window_dt = precice_dt = 0.04

    if interpolation_scheme == ReadWaveformSchemes.LAGRANGE_LINEAR.value:
        configuration_file_name = f"configs/precice-config-{window_dt}.xml"
    elif interpolation_scheme == ReadWaveformSchemes.HERMITE_CUBIC.value:
        configuration_file_name = f"hermite_configs/precice-config-{window_dt}-cubic.xml"

    if participant_name == ParticipantNames.MASS_LEFT.value:
        my_dt = dt
    if participant_name == ParticipantNames.MASS_RIGHT.value:
        my_dt = window_dt
...
filepath = this_file.parent / f"{Cases.MULTIRATE.value}_{participant_name}_{time_stepping_scheme}.csv"
df.to_csv(filepath)
